...
REPOSITORY_MAPPINGS = {
    ...
    # Only packages that fail PyPI auto-discovery or need specific license files
    ...
    "pycparser": "https://github.com/eliben/pycparser/blob/master/LICENSE",
    "typing-extensions": "https://github.com/python/typing_extensions/blob/main/LICENSE",
    "cffi": "https://github.com/python-cffi/cffi/blob/main/LICENSE",
    ...
    # certifi: PyPI returns truncated repo path, use direct URL
        ...
        if response.status_code == 200:
            ...
            if license_url:
                ...
                if license_response.status_code == 200:
                    ...
                    print(
                        f"Warning: Could not download license file for {dep_name} - HTTP {license_response.status_code}")
                    return None
            else:
...
def get_python_dependencies():
    ...
    try:
        ...
        print(
            f"Found {len(package_names)} production Python packages in uv.lock (excluded {excluded} dev dependencies)")
...
def generate_dependency_tracking(all_deps, license_cache):
    ...
    with open(tracking_path, 'w', encoding='utf-8') as f:
        # Header
        f.write(
            f"{'Dependency':<{name_width}} | {'Type':<{type_width}} | {'License':<{license_width}} | {'Classification':<{class_width}}\n")
        f.write(f"{'-' * name_width}-+-{'-' * type_width}-+-{'-' * license_width}-+-{'-' * class_width}\n")

        # Rows
        for dep_name, dep_type, license_name, classification in rows:
            f.write(
                f"{dep_name:<{name_width}} | {dep_type:<{type_width}} | {license_name:<{license_width}} | {classification:<{class_width}}\n")
...
def process_missing_licenses(missing_deps, license_cache):
    ...
    for dep_name, dep_type in missing_deps:
        ...
        if not is_valid:
            errors.append(
                f"License '{license_data['name']}' for {dep_name} is not in LICENSE_PATTERNS or LICENSE_PATTERNS_CONT")
            continue
